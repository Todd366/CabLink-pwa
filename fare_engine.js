...
window.FARE_CONFIG={
  ...
  platformFee:0.10, driverMargin:0.25, maintPerKm:0.55, avgSpeedKmh:45,
...
window.calculateFare=function(km,type,surge){
  ...
  var c=window.FARE_CONFIG,eco=c.fuelEconomy[type]||12;
  var litres=type==='moto'?(km/eco):((km/100)*eco);
  var fuel=litres*c.petrolPerLitre,maint=km*c.maintPerKm;
  var sub=c.baseFare+(km*1.80)+fuel+maint;
  ...
  return{km:km,type:type,litres:+litres.toFixed(3),fuel:+fuel.toFixed(2),
    maint:+maint.toFixed(2),total:total,eta:Math.round((km/c.avgSpeedKmh)*60),surge:surge};
};
