...
// ---------------------------------------------------------------
//
// Many CabLink-style repos have a `vite.config.js` with a `root:`
// option pointing at a subdirectory (e.g. "frontend"). If present,
// THAT directory's index.html — not the repo-root index.html — is
// what actually gets served/built. Guessing wrong here silently
// invalidates every duplicate/ACTIVE/connection-map result, so we
// detect it explicitly and say so out loud.
function detectViteRoot() {
  ...
  for (const name of candidates) {
    ...
    const m = content.match(/\broot\s*:\s*(['"`])([^'"`]+)\1/);
    if (m) {
      const dir = m[2].replace(/^\.\//, '').replace(/\/$/, '');
      return { configFile: name, viteRoot: dir };
    }
    return { configFile: name, viteRoot: null }; // config exists but no explicit root -> repo root
  }
  return { configFile: null, viteRoot: null }; // no vite config found at all
}

const VITE_INFO = detectViteRoot();
// Repo-relative path to the HTML file that is ACTUALLY served/built.
const ENTRY_HTML_REL = VITE_INFO.viteRoot
  ? [VITE_INFO.viteRoot, 'index.html'].join('/')
  : 'index.html';
// True only when we positively confirmed a vite.config with an explicit
// root — i.e. we're CONFIDENT any other .html file is not the live one.
const ENTRY_CONFIRMED = !!VITE_INFO.viteRoot;
...
function resolveScriptSrc(rawSrc) {
  const stripped = rawSrc.replace(/^\//, '');
  const base = VITE_INFO.viteRoot ? VITE_INFO.viteRoot + '/' : '';
  return (base + stripped).split('/').filter(seg => seg !== '.').join('/');
}

function getScriptLoadOrder() {
  const indexPath = path.join(ROOT, ENTRY_HTML_REL);
  if (!fs.existsSync(indexPath)) return { order: [], inlineRanges: [], entryMissing: true };
  ...
  while ((m = scriptTagRe.exec(content)) !== null) {
    ...
    if (srcMatch) {
      ...
      order.push({
        ...
        type: 'src',
        src: srcMatch[1],
        srcResolved: isExternal ? null : resolveScriptSrc(srcMatch[1]),
        isExternal,
        isModule
      });
  ...
  return { order, inlineRanges, entryMissing: false };
}

function annotateLoadOrder(defsByName, scriptOrder) {
  const srcOrderMap = {};
  scriptOrder.order.forEach(entry => {
    if (entry.type === 'src' && entry.srcResolved) {
      srcOrderMap[entry.srcResolved] = entry.order;
    }
  });
  ...
  for (const name of Object.keys(defsByName)) {
    annotated[name] = defsByName[name].map(def => {
      let loadOrder = null;
      let loadedInBrowser = null;
      if (def.file === ENTRY_HTML_REL) {
        loadOrder = 0.5;
        ...
        loadOrder = null;
        loadedInBrowser = ENTRY_CONFIRMED ? false : null;
...
function extractFetchUrlShape(callText) {
  let depth = 0;
  let argEnd = callText.length;
  const openParen = callText.indexOf('(');
  for (let i = openParen; i < callText.length; i++) {
    const c = callText[i];
    if (c === '(') depth++;
    else if (c === ')') { depth--; if (depth === 0) { argEnd = i; break; } }
    else if (c === ',' && depth === 1) { argEnd = i; break; }
  }
  const urlArgText = callText.slice(openParen + 1, argEnd);
  const literalRe = /(['"`])((?:(?!\1)[^\\]|\\.)*)\1/g;
  let m;
  let combined = '';
  let found = false;
  while ((m = literalRe.exec(urlArgText)) !== null) {
    combined += m[2];
    found = true;
  }
...
function scanFrontendFetches(files, defsByName) {
  ...
  for (const file of files) {
    ...
    while ((m = fetchStartRe.exec(content)) !== null) {
      ...
      const urlShape = extractFetchUrlShape(callText);
      if (!urlShape || !urlShape.includes('/api/')) continue;
      const endpoint = urlShape;
...
const COMPLETE_PATTERN = /\/complete(\/|$|\?)/i;
...
function buildConnectionMap(fetchCalls, backendRoutes) {
  ...
  const results = namedFlow.map(step => {
    const frontendCall = fetchCalls.find(c => c.calledFromFunction === step.fn &&
      c.endpoint.includes(step.p) && c.method === step.method);
    const backendMatch = backendRoutes.find(r => r.method === step.method && r.path === step.p);
    ...
    return { step: step.step, frontendCall: frontendCall || null, backendRoute: backendMatch || null, status };
  });

  const acceptFrontend = fetchCalls.find(c => ACCEPT_PATTERN.test(c.endpoint));
  const acceptBackend = backendRoutes.find(r => ACCEPT_PATTERN.test(r.path));
  results.push({
    step: 'ride acceptance (any function) -> accept endpoint',
    frontendCall: acceptFrontend || null,
    backendRoute: acceptBackend || null,
    status: acceptFrontend && acceptBackend ? 'GREEN' : (acceptFrontend || acceptBackend) ? 'YELLOW' : 'RED'
  });

  const completeFrontend = fetchCalls.find(c => COMPLETE_PATTERN.test(c.endpoint));
  const completeBackend = backendRoutes.find(r => COMPLETE_PATTERN.test(r.path));
  results.push({
    step: 'ride completion (any function) -> complete endpoint',
    frontendCall: completeFrontend || null,
    backendRoute: completeBackend || null,
    status: completeFrontend && completeBackend ? 'GREEN' : (completeFrontend || completeBackend) ? 'YELLOW' : 'RED'
  });
...
function scanDeadCode(allProjectFiles, scriptOrder) {
  const referencedSrc = new Set(
    scriptOrder.order.filter(e => e.type === 'src' && e.srcResolved).map(e => e.srcResolved)
  );
...
function buildTruthReportMarkdown(graph) {
  ...
  lines.push(`Resolved entry HTML: ${graph.meta.entryHtml}${graph.meta.entryConfirmed ? ' (confirmed via ' + graph.meta.viteConfigFile + ')' : ' (UNCONFIRMED — verify manually)'}`);
  lines.push('');
  lines.push('_Note: only `<script src>` tags are traced. ES module import graphs (e.g. main.jsx importing App.jsx and further components) are not followed — a function that only lives inside an imported .jsx file may show as "not loaded in browser" below even though bundling would include it. Treat .jsx-only findings as unverified, not confirmed dead._');
  lines.push('');
...
async function main() {
  ...
  if (VITE_INFO.configFile) {
    console.log(`Vite config detected: ${VITE_INFO.configFile}`);
    console.log(`  root: ${VITE_INFO.viteRoot || '(not set — defaults to repo root)'}`);
    console.log(`  entry HTML resolved to: ${ENTRY_HTML_REL}${ENTRY_CONFIRMED ? '' : ' (UNCONFIRMED — no explicit root: in config, treating repo-root index.html as entry but NOT ruling out other .html files)'}`);
  } else {
    console.log('No vite.config.js/.mjs/.ts found — assuming index.html at repo root is the entry point.');
    console.log('If this project uses a different bundler or a non-standard entry, script-load-order and duplicate/ACTIVE results below may be wrong. Verify manually.');
  }
  console.log('NOTE: this tool follows <script src="..."> tags only. It does NOT trace');
  console.log('ES module import graphs (e.g. main.jsx -> App.jsx -> ...). A function');
  console.log('defined only inside an imported .jsx component will show as "not loaded');
  console.log('in browser" even if it truly is, once bundled. Treat .jsx-only definitions');
  console.log('as unverified rather than confirmed dead.');
  console.log('');
  ...
  const graph = {
    meta: {
      ...
      viteConfigFile: VITE_INFO.configFile,
      viteRoot: VITE_INFO.viteRoot,
      entryHtml: ENTRY_HTML_REL,
